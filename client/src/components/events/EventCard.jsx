...
export default function EventCard({
  ...
  return (
    <div className="card p-6">
      <h3 className="text-lg font-semibold text-espresso">{event.title}</h3>
      ...
      <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-cocoa/60">
            ...
            {isRegistered ? (
              <span className="pill">You are registered</span>
            ) : null}
          </div>
        </div>
        ...
        <button
          onClick={() =>
            isRegistered ? onUnregister?.(event) : onRegister?.(event)
          }
