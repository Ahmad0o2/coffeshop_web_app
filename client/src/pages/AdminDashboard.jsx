                ...
                <div className="flex items-center justify-between gap-2">
                  <span
                    className={cn(
                      ...
                      isDayTheme ? "text-espresso" : "text-espresso",
                    )}
                  >
                    {tab.label}
                  </span>
...
export default function AdminDashboard() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const basePrice = Math.min(
        ...normalizedSizes.map((entry) => entry.price),
      );
    ...
    try {
      ...
      if (draft.trackInventory) {
        if (draft.inventoryQuantity === "") {
          throw new Error(
            "Enter the stock count or disable inventory tracking.",
          );
        }
        ...
        if (!Number.isInteger(inventoryQuantity) || inventoryQuantity < 0) {
          throw new Error(
            "Inventory count must be a whole number of 0 or more.",
          );
        }
      }
      ...
      if (!Number.isInteger(lowStockThreshold) || lowStockThreshold < 0) {
        throw new Error(
          "Low-stock warning must be a whole number of 0 or more.",
        );
      }
  ...
  const handleRewardEdit = (reward) => {
    setRewardForm({
      id: reward._id,
      productId:
        reward.product?._id || reward.productId?._id || reward.productId || "",
  ...
  const saveFeatured = async () => {
    ...
    try {
      ...
      formData.append(
        "featuredEventIds",
        JSON.stringify(normalizedFeaturedIds),
      );
      const { data } = await api.put("/admin/settings", formData);
      if (data?.settings) {
        queryClient.setQueryData(["settings"], data.settings);
  ...
  const activeEventsCount = adminEvents.filter(
    ...
    return inventoryProducts.filter((product) => {
      const matchesSearch = search
        ? [product.name, categoryMap.get(product.categoryId) || ""]
            ...
            .includes(search)
        : true;
      const matchesStatus = matchesInventoryStatusFilter(
        product,
  ...
  const activeDashboardStat = (() => {
    const statKey =
      activeTab === "manage"
        ? "team"
        : ["orders", "products", "inventory", "rewards", "events"].includes(
              activeTab,
            )
          ? activeTab
          : null;
  ...
  const activeTabMeta =
    dashboardTabs.find((tab) => tab.key === activeTab) ||
    dashboardTabs[0] ||
    null;
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-cocoa/70">
                ...
                and team access, with the same functionality and better
                structure.
              </p>
            </div>
          </div>
      ...
      {isMobileNavOpen && (
        <div className="fixed inset-0 z-[70] xl:hidden">
          ...
          <div className="absolute inset-y-0 left-0 w-[84vw] max-w-[21rem] p-3">
            <div
              className={cn(dashboardSidebarClass, "h-full rounded-[2rem] p-4")}
            >
          ...
          <div className={dashboardSidebarFrameClass}>
            <div className="flex h-full flex-col pt-12">
              <button
                ...
                aria-label={
                  isNavCompact ? "Expand navigation" : "Collapse navigation"
                }
                title={
                  isNavCompact ? "Expand navigation" : "Collapse navigation"
                }
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid gap-3">
                      {sizeOptions.map((size) => {
                        const entry = form.sizePrices.find(
                          (item) => item.size === size,
                        );
                        return (
                          <div
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <p className="text-[11px] text-cocoa/60">
                          When left empty, this product stays orderable without
                          stock tracking.
                        </p>
                      </div>
                      <div className="space-y-2">
                        ...
                        />
                        <p className="text-[11px] text-cocoa/60">
                          Show a low-stock state once the count reaches this
                          number.
                        </p>
                      </div>
                    </div>
              ...
              <div className={dashboardPanelClass}>
                ...
                >
                  <p className="text-xs font-semibold text-espresso">
                    Showing {filteredInventoryProducts.length} of{" "}
                    {inventoryProducts.length}
                  </p>
                  <p className="text-[11px] leading-5 text-cocoa/60">
                    Orders reduce tracked stock immediately. Cancelling an order
                    or removing an item from a received order restores the
                    reserved quantity.
                  </p>
                </div>
              </div>
            ...
            <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
              ...
              >
                ...
                <div className="mt-5 space-y-4 text-sm">
                  {products.length === 0 && (
                    <div className="rounded-xl2 border border-dashed border-gold/20 bg-obsidian/45 p-4 text-sm text-cocoa/60">
                      No menu items found yet. Add products first, then create
                      rewards from them.
                    </div>
                  )}
                  <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 line-clamp-2 text-xs text-cocoa/60">
                            {selectedRewardProduct.description ||
                  ...
                  {adminRewards.map((reward) => (
                    <div key={reward._id} className={dashboardItemClass}>
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        ...
                        <Badge>
                          {reward.isActive === false ? "Inactive" : "Active"}
                        </Badge>
                      </div>
                      {reward.description && (
                  ...
                  <div className="upload-panel">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-cocoa/60">
                          Separate from the Gallery page. Upload up to 8 images
                          that appear only in the Home showcase section.
                        </p>
                      </div>
                      <label className="upload-button">
                        <input
          ...
          {activeTab === "events" && (
            <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
              ...
              <div className={dashboardPanelClass}>
                ...
                />
                {featuredError && (
                  <p className="form-error mt-3">{featuredError}</p>
                )}
                <div className="mt-4 space-y-3 text-sm">
          ...
          {activeTab === "gallery" && (
            <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
              <div className={dashboardPanelClass}>
                ...
                {galleryError && (
                  <p className="form-error mt-4">{galleryError}</p>
                )}
                  ...
                  {staffList.map((member) => (
                    <div key={member.id} className={dashboardItemClass}>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        ...
                        <Button
                          variant="outline"
                          onClick={() => handleStaffDelete(member)}
                        >
                          Remove
                        </Button>
                      </div>
