...
- Access tokens stay in memory; refresh tokens are rotated through an `httpOnly` cookie
...
### Events
- The events page is publicly viewable for SEO and sharing
- Event registration and unregister flows still require sign-in
...
## SEO Notes
- `HelmetProvider` is wired in `client/src/main.jsx`
- Main pages set route-specific `<title>` and description tags
- Home emits `Restaurant` JSON-LD
- Product detail emits `Product` JSON-LD
- `client/public/robots.txt` blocks `/admin` and `/sign-in`
- The sitemap is generated by the backend at `/sitemap.xml`
...
- `client/src/pages/AdminDashboard/index.jsx`
...
- React Query defaults are centralized in `main.jsx`, with stale data disabled on window focus by default.
